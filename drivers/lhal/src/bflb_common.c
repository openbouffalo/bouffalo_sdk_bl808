...
#define ANTI_ROLLBACK_BOOT2_VERSION_OFFSET (0x34)
#define ANTI_ROLLBACK_APP_VERSION_OFFSET   (0x38)
#endif
...
const bflb_verinf_t app_ver __attribute__ ((section(".bflb_verinf"))) = {
#ifndef CONFIG_APP_ANTI_ROLLBACK_VER
    .anti_rollback = 0,
#else
...
bool bflb_check_cache_addr(const void *addr)
{
    ...
    if ((a & 0xFFFF0000UL) >= 0x60FC0000UL) {
        return true;
    }
    return false;
}
...
bool bflb_check_cache_addr(const void *addr)
{
    ...
    if ((a & 0xFFF00000UL) >= 0x61000000UL) {
        return true;
    }
    return false;
}
...
bool bflb_check_cache_addr_aligned(uintptr_t addr)
{
    ...
    return romapi_bflb_check_cache_addr_aligned(addr);
#else
...
__WEAK int32_t bflb_get_app_version_from_efuse(uint8_t *version)
{
    ...
    version_low = value[1];
    version_high = value[2];
    version_high_high = value[3];
#endif
...
__WEAK int32_t bflb_get_boot2_info_from_flash(bflb_verinf_t *version)
{
    uint32_t addr = 0xC08;
    bflb_flash_read(addr + BFLB_BOOT2_IMG_OFFSET, (uint8_t *)version, sizeof(bflb_verinf_t));
